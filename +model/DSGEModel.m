...
% abstract superclass for model
    ...
    methods
        ...
        function obj=DSGEModel(params,endogenv,exogenv,vfct,pfct,tfct,ffct)
        ...
        % evaluate forecasting function
        function stvec=evaluateForec(obj,point)
            stvec=obj.Ffct.evaluateAt(point);
        end
        
        % solution at point based on abstract methods
        function [fx,Jac,V]=solveAtPoint(obj,solvec,point,mode,trans,Polnext,checkTrans)
            % transition
            [nextst,outstr]=obj.calcStateTransition(point,solvec,0,trans);
            % equations
            [fx,Jac,V]=obj.calcEquations(point(1),nextst,solvec,outstr,mode,checkTrans,Polnext);
        end        
        ...
        function obj=updateFfct(obj,vals)
            obj.Ffct=obj.Ffct.fitTo(vals);
        end
        ...
        function [resmat,VF,TF,FF,failedPointsIndex]=solvePointList(mobj,tmp_grid,tmp_resmat,tmp_transmat,...
                tmp_refidx,Vtrans,tmp_resmat_prev,print,logfname,checkTrans)
            
            
            % Initializing the vectors that I will need afterwards.
            gr_npt=size(tmp_grid,1);
            VF=zeros(gr_npt,mobj.Vfct.Nof);
            TF=zeros(gr_npt,mobj.Tfct.Nof);
            FF=zeros(gr_npt,mobj.Ffct.Nof);
            resmat=tmp_resmat;
...
%           for i=1:gr_npt
                ...
                % THIS FUNCTION IS AT DSGEModel.m
                fh_solveAtPoint=@(sol)solveAtPoint(mobj,sol,point,0,trans,Polnext,checkTrans);
                ...
                if exfl<1 || sum(isnan(fx)) > 0
                    ...
                    % If there is no problem just save x as the solution.
                    xr=x;
                end
                
                % Record result in the i-th row, we will do this the number
                % of combination of non-exogenous state variables there are.
                resmat(i,:)=xr(1:mobj.NSOL);
                
                % Also compute VF at solution
                [~,~,V]=solveAtPoint(mobj,xr,point,1,trans,Polnext,0);
        ...
        % solve model at list of points
        function [resmat_failed,VF_failed,TF_failed,FF_failed,n_succ]=solvePointListFailed(mobj,grid,index_failed,resmat,...
                tmp_transmat, tmp_refidx,Vtrans, npasses,print,logfname,checkTrans)
            ...
            FF_failed=zeros(length(index_failed),mobj.Ffct.Nof);
...
%                 'MaxIter',100,'MaxFunEvals',20000,'Algorithm','trust-region-dogleg');            
            ...
            for p=1:npasses
                ...
                this_FF_failed=zeros(length(index_failed),mobj.Ffct.Nof);
...
%                    if exfl>=0
                    ...
                    % record result
                    this_rmfailed(i,:)=xr(1:mobj.NSOL);
                ...
                FF_failed(failed_idx_list,:)=this_FF_failed;
